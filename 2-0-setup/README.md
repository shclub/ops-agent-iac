...
7. 공유 모니터링 스택 `<project>-monitoring-server`(t3.small) — docker compose로
   ...
   시 함께 provisioning된다. **대시보드는 이 서버가 아니라 `2-0-setup/3-grafana`
...
> `2-0-setup/4-hermes/README.md` 참고. 아래는 그 중 read 자격증명(토큰) 발급 부분이다.
...
#    (3-grafana)를 지운다. foundation을 먼저 내리면 서버가 없어 이 destroy가 접속
