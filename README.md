...
│                        #   3-grafana(대시보드·토큰) · 4-hermes · teardown.sh
...
# (8) 에이전트 배선 — ops 플러그인(read + write) 을 hermes 호스트에 설치
ssh -i ~/.ssh/ops-agent-iac ubuntu@<hermes 호스트 public IP>   # 1-foundation output hermes_host_public_ip
# 호스트에서 — 플러그인 설치
hermes plugins install <OWNER>/ops-agent-plugin   # <OWNER> = 본인 GitHub 핸들
