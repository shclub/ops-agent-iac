---
name: fc-e2e-live
description: ops-agent 시나리오를 실제로 end-to-end 실행하라는 요청에 사용 — "e2e 돌려줘", "시나리오 전부 실제로 돌려", "라이브로 검증해줘", 수업 전, 또는 정적 리뷰만으론 못 믿을 만큼 큰 변경 이후. Slack MCP로 Slack의 Hermes에게 실제 프롬프트를 보내고 실제 부수효과(PR → merge → apply → 라이브 AWS/Cloudflare)를 검증한다. 읽기 전용 상태 확인(fc-deploy-verify-phase1/phase2)이나 코드-의도 대조 리뷰(fc-intent-review)에는 쓰지 않는다.
---
...
- **Slack plugin 자동 부트스트랩:** 먼저 현재 세션의 도구 목록에서 `slack_search_channels`·`slack_send_message`·`slack_read_channel`·`slack_read_thread`를 찾고 `codex plugin list`로 설치 상태를 조회한다. `slack@openai-curated`가 `not installed`면 blocker로 끝내지 말고 `codex plugin add slack@openai-curated`로 설치한다. CLI 설치가 불가능한 환경에서만 Codex/ChatGPT의 **Plugins** 탭에서 Slack plugin을 설치한다. 이 E2E 요청은 plugin 설치·Slack OAuth 연결 시도를 승인한 것으로 본다.
- **관리형 OAuth만 사용:** plugin 설치는 인증 완료가 아니다. `slack@openai-curated`가 제공하는 Slack App 연결에서 `Connect`를 열어 대상 코스 워크스페이스를 OAuth 승인한다. 일반 경로에서 raw `https://mcp.slack.com/mcp`를 `codex mcp add/login`으로 별도 등록하거나 Slack client ID/client secret·bot token·redirect URI를 사용자에게 요구하지 않는다. 로그인 또는 Slack 관리자 승인 화면처럼 에이전트가 대신 완료할 수 없는 단계만 사용자에게 요청하고, 승인 직후 같은 런을 재개한다.
- **두 Slack 연결을 구분:** [Codex Settings → Connectors](https://chatgpt.com/codex/settings/connectors)의 `<연결된 워크스페이스>`/`Disconnect` 표시는 Slack에서 `@Codex`로 cloud chat을 시작하는 **인바운드 Codex Slack 앱** 연결이다. 이것만으로 로컬 Codex 세션에 위 4개 Slack 송수신 도구가 생기지 않으며, Plugin Management의 `not_installed`와 모순되지 않는다. E2E의 충족 조건은 Slack plugin이 설치·OAuth 연결되고 실제 도구 4개가 호출되는 것이다. **Claude/Anthropic OAuth 또는 `~/.claude*` Slack 설정도 대체가 아니므로 사용하지 않는다.**
- **설치 후 세션 갱신:** plugin이 `installed, enabled`인데 Slack 도구가 없으면 설치 실패로 오판하지 않는다. 새 plugin의 도구는 새 채팅에서 로드되므로 체크리스트를 보존한 뒤 Codex를 reload하거나 새 세션에서 즉시 재개한다. 재개 세션에서는 설치 여부를 다시 묻지 말고 `slack_list_workspaces`(있으면) 또는 `slack_search_channels`를 먼저 호출한다. 인증 프롬프트가 뜨면 위 관리형 OAuth를 완료하고 같은 도구를 재호출한다. `not_installed`/인증 실패를 사용자에게 떠넘기기 전에 CLI 설치·OAuth·reload·재호출을 실제로 시도한 증거를 체크리스트에 남긴다.
- **Slack MCP 가동 확인:** `slack_list_workspaces`가 있으면 OAuth로 연결한 코스 워크스페이스와 team ID를 확인하고, `slack_search_channels`로 `#ops-agent` 채널 ID를 확보한 뒤 `slack_read_channel` 실호출까지 성공시킨다. 봇 user ID(`U…`)는 **가장 최근 봇 발화**에서 추출한다 — `slack_search_users`는 봇 계정을 반환하지 않는다(실측). 옛 사람-멘션(`<@U…|ops-agent>`)만으로 정하지 않는다: Slack 앱을 재설치하면 봇 user ID가 바뀌어 히스토리의 옛 ID가 stale이 된다(실측 — 죽은 ID 멘션은 봇을 트리거하지 않고 무응답으로만 나타난다). 봇 발화가 없거나 히스토리가 비어 있으면 hermes 호스트 `.env`의 봇 토큰으로 `auth.test`를 호출해 확정하거나(`user_id` 필드), 사용자에게 봇 프로필의 member ID 복사를 요청한다. 멘션은 `<@U…>` 포맷만 봇을 트리거하고, 일반 텍스트 `@Ops Agent`는 트리거하지 않는다. 설치 상태나 도구 이름만 확인한 것은 OAuth 검증이 아니다.
...
| # | merge+apply 후 검증할 것 |
|---|---|
| 1 세팅 | tf-apply success; EC2 bastion+app-0/1 running; ALB `/healthz` 200 (기본 기반은 fc-deploy-verify-phase1, Grafana/PD는 phase2) |
...
| "Slack이 `not_installed`니까 사용자에게 설치하라고 하고 끝내자" | `codex plugin add slack@openai-curated` → 관리형 OAuth → reload → workspace/channel 실조회까지 진행한다. raw MCP용 client ID/secret은 일반 경로에서 요구하지 않는다. `@Codex` 인바운드 연결 화면을 송수신 plugin으로 오인하지 않는다. |
