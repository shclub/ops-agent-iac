...
#   2-06 waf.auto.tfvars.json     → waf_rules           (존 전역 WAF — prod 전용 surface, waf.tf)
...
# --- 2-06 waf.auto.tfvars.json: 특정 IP 차단(WAF 커스텀 룰, 존 전역) ----------
# 에이전트 write surface — prod 전용(2026-07-20 결정). 차단은 존 전역으로 적용된다
# (dev 호스트 포함). 리소스는 waf.tf의 cloudflare_ruleset.waf.
variable "waf_rules" {
  description = "Cloudflare WAF 커스텀 룰 맵(존 전역). 논리 이름 → {ip, action, path?}. 특정 소스 IP(단일 또는 CIDR)를 차단/챌린지. action: block|managed_challenge|js_challenge(기본 block)."
  type = map(object({
    ip     = string
  ...
  validation {
    condition     = alltrue([for r in var.waf_rules : can(cidrhost(strcontains(r.ip, "/") ? r.ip : "${r.ip}/32", 0))])
    error_message = "waf_rules의 ip는 유효한 IP 또는 CIDR이어야 한다(예: 203.0.113.45 또는 203.0.113.0/24)."
  }

  validation {
    condition     = alltrue([for r in var.waf_rules : strcontains(r.ip, "/") ? tonumber(split("/", r.ip)[1]) > 0 : true])
    error_message = "waf_rules의 ip는 /0(0.0.0.0/0·::/0)일 수 없다(사이트 전체 차단 방지)."
  }

  validation {
    condition     = alltrue([for r in var.waf_rules : contains(["block", "managed_challenge", "js_challenge"], r.action)])
    error_message = "waf_rules의 action은 block, managed_challenge, js_challenge 중 하나여야 한다."
  }

  validation {
    condition     = alltrue([for r in var.waf_rules : r.path == "" || can(regex("^/[A-Za-z0-9/_.~-]*$", r.path))])
    error_message = "waf_rules의 path는 /로 시작하고 [A-Za-z0-9/_.~-]만 허용한다(표현식 인젝션 방지)."
  }
}
